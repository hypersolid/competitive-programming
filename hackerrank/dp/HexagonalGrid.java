...
public class Solution {
    ...
    static boolean applies2(int[][] dp, int i, int j) {
        if (j == 0)
            return i - 1 >= 0 && dp[1][i - 1] == 0;
        return i + 1 < N && dp[0][i + 1] == 0;
    }

    static boolean applies(int[][] dp, int i, int j, int tile) {
        switch (tile) {
            case 0: return applies0(dp, i, j);
            case 1: return applies1(dp, i, j);
            case 2: return applies2(dp, i, j);
        }
        return false;
    }
    ...
    static int[][] apply(int[][] dp, int i, int j, int tile) {
        ...
        switch (tile) {
            case 0: dpC[(j+1) % 2][i] = tile + 1; break;
            case 1: dpC[j][i + 1] = tile + 1; break;
            case 2: {
