...
public class Solution {
  ...
  private static void propogate(int n, int m) {
    if (m >= 1) memo[n][m] += memo[n - 1][m - 1];
    memo[n][m] += memo[n - 1][m];
    if (m < M - 1) memo[n][m] += memo[n - 1][m + 1];
    memo[n][m] %= MODULO;
  }
  ...
  private static void solve() {
    ...
    if (A[0] == -1) Arrays.fill(memo[0], 1);
    else memo[0][A[0]] = 1;

    for (int n = 1; n < N; n++) {
      if (A[n] == -1) {
        for (int m = 0; m < M; m++) propogate(n, m);
      } else {
        propogate(n, A[n]);
      }
    }
    ...
    long result = 0;
    for (int i = 0; i < M; i++) result += memo[N - 1][i];
