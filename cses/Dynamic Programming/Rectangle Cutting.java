...
public class Solution {
  ...
  private static int dp(int m, int n) {
    ...
    for (int i = 1; i < m; i++) result = Math.min(result, dp(i, n) + dp(m - i, n) + 1);
    for (int i = 1; i < n; i++) result = Math.min(result, dp(m, i) + dp(m, n - i) + 1);
  ...
  private static void solve() {
    ...
    for (int i = 0; i <= A; i++) Arrays.fill(memo[i], -1);
    for (int i = 0; i <= Math.min(A, B); i++) memo[i][i] = 0;
