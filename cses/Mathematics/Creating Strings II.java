...
public class Solution {
  ...
  private static final long LIMIT = (long) Math.pow(10, 6);

  private static FastReader reader = new FastReader();
  private static FastWriter writer = new FastWriter();

  private static String S;
  private static long[] factorial = new long[(int) LIMIT + 1];
  ...
  private static void solve() {
    ...
    long permutatuins = factorial[S.length()];
    long repetitions = 1;
    for (int i = 0; i < 26; i++) {
      if (chars[i] == 0) continue;
      repetitions = repetitions * factorial[chars[i]] % MODULO;
    }

    long inverse = modInverse(repetitions, MODULO);
    if (inverse < 0) inverse += MODULO;
    writer.println(permutatuins * inverse % MODULO);
  }
