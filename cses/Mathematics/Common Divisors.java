...
public class Solution {
  ...
  private static void solve() {
    ...
    for (int k = N - 1; k >= 0; k--) {
      for (int i = 1; i < Math.sqrt(A[k]); i++) {
        if (i < ptr && A[k] / i < ptr) break;

        if (A[k] % i == 0) {
          memo[i]++;
          memo[A[k] / i]++;

          if (memo[i] > 1) ptr = Math.max(ptr, i);
          if (memo[A[k] / i] > 1) ptr = Math.max(ptr, A[k] / i);
        }
      }
      //      debug(A[k] + " " +ptr);
      if (Math.sqrt(A[k]) % 1 == 0) {
        int divisor = (int) Math.sqrt(A[k]);
        memo[divisor]++;
        if (memo[divisor] > 1) ptr = Math.max(ptr, divisor);
      }
    }
