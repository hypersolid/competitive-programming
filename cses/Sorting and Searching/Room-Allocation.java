...
public class Solution {
  ...
  private static PriorityQueue<Quad> q =
      new PriorityQueue<>((a, b) -> Integer.compare(a.val2, b.val2));
  private static ArrayList<Quad> guestsInitial;
  private static ArrayList<Quad> guests;

  private static void readInput() throws IOException {
    N = scanner.nextInt();
    guests = new ArrayList<>(N);
    guestsInitial = new ArrayList<>(N);
  }
  ...
  private static void solve() throws IOException {
    for (int i = 0; i < N; i++) {
      ...
      Quad guest = new Quad(arrival, departure, -1, i);
      guests.add(guest);
      guestsInitial.add(guest);
    }
    Collections.sort(guests, (a, b) -> Integer.compare(a.val1, b.val1));

    int maxRoom = 0;
    for (Quad guest : guests) {
      int arrival = guest.val1;
      int departure = guest.val2;
      int room = q.size() + 1;

      Quad nextDeparture = q.peek();
      if (nextDeparture != null && nextDeparture.val2 < arrival) room = q.poll().val3;
    ...
    log.println(maxRoom);
    for (Quad guest : guestsInitial) {
      log.print(guest.val3);
      log.print(" ");
    }
  }
