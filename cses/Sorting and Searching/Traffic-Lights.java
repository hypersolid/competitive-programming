...
public class Solution {
    ...
    private static IMultiset intervals = new IMultiset();
    private static TreeSet<Integer> positions = new TreeSet<>();
    ...
    private static void solve() throws IOException {
        ...
        for(int i = 0; i < N; i++) {
            ...
            int l = positions.lower(light);
            int r = positions.higher(light);
            int diff = r - l;

            positions.add(light);

            intervals.remove(diff);
            if (light - l > maxInterval / 10) intervals.add(light - l);
            if (r - light > maxInterval / 10) intervals.add(r - light);

            if (diff == maxInterval) maxInterval = intervals.lastKey();
