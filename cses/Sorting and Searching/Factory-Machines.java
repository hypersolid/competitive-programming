...
public class Solution {
  ...
  private static void readInput() throws IOException {
    ...
    machines = scanner.fillIntegerArray(N);
  }

  private static boolean canProduce(long time) throws IOException {
    long products = 0;
    for (int machine : machines) products += time / machine;
    return products >= T;
  }

  private static void solve() throws IOException {
    long l = 1;
    long r = Long.MAX_VALUE / N;
    while (l < r) {
      long mid = l + (r - l) / 2;
      if (canProduce(mid)) {
        r = mid;
